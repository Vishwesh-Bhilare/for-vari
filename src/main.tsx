  ...
  return (
    <main className="min-h-screen bg-saffron-50 text-stone-900">
      ...
      {view === 'admin' ? <div className="min-h-screen bg-saffron-50 px-4 py-3 pb-20 lg:px-8 lg:pb-4"><h2 className="mb-3 text-xl font-extrabold text-stone-900">Admin dashboard</h2>{role === 'admin' && <><p className="mb-3 text-sm text-stone-600">Live locations for all pilgrims who have granted device permission.</p><div id="map" className="mb-4 h-[320px] w-full overflow-hidden rounded-2xl bg-cream-100" /></>}<AdminLogin userId={currentMemberId} userEmail={session?.user?.email} role={role} activeSosCount={activeSosCount} registeredProfileCount={registeredProfileCount} routeStationCount={nodes.length} nodes={nodes} broadcastMessages={activeBroadcasts} onNodesChange={setNodes} onBroadcastCreated={(broadcast) => setBroadcastMessages((messages) => [broadcast, ...messages.filter((message) => message.id !== broadcast.id)].slice(0, 3))} onDeleteBroadcast={(id) => setBroadcastMessages((messages) => messages.filter((m) => m.id !== id))} /></div> : view === 'news' ? <div className="min-h-screen bg-saffron-50 p-4 pb-20 max-w-7xl mx-auto lg:p-6"><LiveNews /></div> : view === 'helplines' ? <div className="min-h-screen bg-saffron-50 p-4 pb-20 max-w-7xl mx-auto lg:p-6"><EmergencyContacts /></div> : view === 'sos_mesh' ? <div className="min-h-screen bg-stone-950 pb-20 lg:pb-4"><VarkariSosMesh currentMemberId={currentMemberId} profile={profile} position={position} nodes={nodes} nearestNodeId={nearestNodeId} onSosCreated={(newAlert) => setSosAlerts((prev) => [newAlert, ...prev])} onRequestLocationPermission={requestLocation} geoError={geoError} onSetManualLocation={setManualLocation} /></div> : <div className="pb-20 lg:pb-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
            <div>
              <span className="text-[11px] font-black uppercase tracking-widest text-amber-200 bg-black/20 px-3 py-1 rounded-full border border-white/20">
                🚩 पंढरपूर आषाढी वारी सोहळा २०२६
              </span>
              <h2 className="mt-2 text-2xl sm:text-3xl font-black tracking-tight text-white">
                {guestLanguage === 'mr' ? 'सुरक्षित वारी प्रवासाचा साथीदार' : 'Plan Your Safe Wari Journey'}
              </h2>
              <p className="mt-1 text-xs sm:text-sm text-amber-100 font-medium max-w-xl">
                {guestLanguage === 'mr'
                  ? 'देहू ते पंढरपूर थेट गर्दी, वाहतूक स्थिती, आपत्कालीन हेल्पलाइन व ऑफलाईन मेश नेटवर्क सहाय्य.'
                  : 'Live crowd density, traffic alerts, emergency helplines & offline peer mesh relay from Dehu to Pandharpur.'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => changeView('sos_mesh')}
                className="px-4 py-2.5 rounded-2xl bg-red-600 hover:bg-red-700 text-white text-xs font-black shadow-md active:scale-95 transition border border-white/30 flex items-center gap-1.5"
              >
                <span>🚨</span>
                <span>SOS Mesh</span>
              </button>
        ...
        {!session && <section className="mx-4 mt-4 rounded-3xl bg-stone-950 p-6 text-white shadow-xl border border-stone-800"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-black">{guestCopy.title}</h2><p className="mt-1 text-sm text-stone-300">{guestCopy.description}</p></div><div className="flex rounded-xl border border-stone-700 bg-stone-900 p-1" aria-label="Translate page language"><button type="button" onClick={() => setGuestLanguage('en')} className={`rounded-lg px-3 py-1.5 text-xs font-bold ${guestLanguage === 'en' ? 'bg-saffron-600 text-white' : 'text-stone-300'}`}>English</button><button type="button" onClick={() => setGuestLanguage('mr')} className={`rounded-lg px-3 py-1.5 text-xs font-bold ${guestLanguage === 'mr' ? 'bg-saffron-600 text-white' : 'text-stone-300'}`}>मराठी</button></div></div><p className="mt-3 rounded-2xl border border-saffron-500/30 bg-saffron-500/10 p-3.5 text-xs font-bold text-saffron-100">{guestCopy.authRequired}</p><div className="mt-4 flex flex-wrap gap-2.5"><button onClick={() => setShowAuthModal(true)} className="rounded-2xl bg-saffron-600 hover:bg-saffron-700 px-5 py-2.5 text-sm font-black shadow-md">{guestCopy.signIn}</button><button onClick={() => changeView('news')} className="rounded-2xl bg-stone-800 border border-stone-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-stone-700">{guestCopy.news}</button><button onClick={() => changeView('helplines')} className="rounded-2xl bg-red-600 hover:bg-red-700 px-4 py-2.5 text-sm font-bold text-white">📞 Helplines</button><button onClick={() => setShowApplyModal(true)} className="rounded-2xl border border-stone-600 px-4 py-2.5 text-sm font-bold hover:bg-stone-800">{guestCopy.volunteer}</button></div></section>}
        
        <div className="lg:grid lg:grid-cols-[2fr_1fr] lg:gap-6 lg:p-4 mt-4">
          <div className="space-y-4">
            <div className="relative mx-4 h-[260px] overflow-hidden rounded-3xl lg:mx-0 lg:h-[420px] shadow-lg border border-stone-200/80">
              <div id="map" className="h-full w-full bg-cream-100" />
              <div className="absolute bottom-3 right-3 flex flex-col items-end gap-2">
                {role === 'admin' && <button onClick={() => setShowTrafficSheet(true)} className="min-h-[44px] rounded-xl bg-stone-900/90 backdrop-blur-md px-3.5 py-2 text-xs font-bold text-white shadow-md hover:bg-stone-900 transition">🚦 Report traffic</button>}
                {role === 'admin' && <button onClick={() => setShowDensitySheet(true)} className="min-h-[44px] rounded-xl bg-saffron-600/90 backdrop-blur-md px-3.5 py-2 text-xs font-bold text-white shadow-md hover:bg-saffron-600 transition">Report density</button>}
                {session && profile?.group_id && <button onClick={() => setShowAddNodeSheet(true)} className="min-h-[44px] rounded-xl bg-violet-600/90 backdrop-blur-md px-3.5 py-2 text-xs font-bold text-white shadow-md hover:bg-violet-600 transition">📌 Add group node</button>}
              </div>
            </div>

            {/* Mobile Horizontal Density Chips */}
            <div className="flex gap-3 overflow-x-auto px-4 pb-2 scrollbar-hide lg:hidden">
              {latestReports.map(({node,density}) => (
                <div key={node.id} className={`w-[140px] flex-shrink-0 rounded-2xl border border-cream-200 border-l-4 bg-white p-3 shadow-sm ${density === 'high' ? 'border-l-red-600' : density === 'medium' ? 'border-l-amber-500' : 'border-l-green-600'}`}>
                  <p className="truncate text-sm font-extrabold text-stone-900">{node.name}</p>
                  <span className={`mt-2 inline-block rounded-full px-2.5 py-1 text-xs font-bold ${density === 'high' ? 'bg-red-50 text-red-700' : density === 'medium' ? 'bg-amber-50 text-amber-700' : density === 'low' ? 'bg-green-50 text-green-600' : 'bg-cream-100 text-stone-500'}`}>{density === 'unknown' ? 'No data' : density}</span>
                </div>
              ))}
            </div>

            {/* Main Interactive Panels Grid */}
            <section className="mx-4 overflow-hidden rounded-3xl border border-stone-200/80 bg-white lg:mx-0 lg:grid lg:grid-cols-3 lg:gap-4 lg:border-0 lg:bg-transparent shadow-sm">
              {(session ? ([['lending','🤝','Peer lending'],['lost','🔎','Lost & found'],['volunteer','🙋','Volunteer application']] as const) : ([['lost','🔎','Lost & found'],['volunteer','🙋','Volunteer application']] as const)).map(([key,icon,title]) => (
                <div key={key} className="border-b border-cream-200 last:border-0 lg:rounded-3xl lg:border lg:border-stone-200/80 lg:bg-white lg:p-5 lg:shadow-md lg:hover-lift">
                  <button onClick={() => setActivePanel(key)} className="flex min-h-[48px] w-full items-center justify-between px-4 py-3 lg:pointer-events-none lg:px-0 lg:py-0">
                    <span className="flex items-center gap-2 text-sm font-black text-stone-900">
                      <span className="text-xl">{icon}</span>{title}
                    </span>
                    <span className={`text-stone-400 transition-transform duration-200 lg:hidden ${activePanel === key ? 'rotate-180' : ''}`}>⌄</span>
                  </button>
                  <div className={`overflow-hidden transition-all duration-200 ease-out ${activePanel === key ? 'max-h-[700px]' : 'max-h-0'} lg:max-h-[900px]`}>
                    <div className="px-4 pb-4 pt-2 lg:px-0">
                      {key === 'lending' ? (
                        <>
                          <div className="mb-3 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                            {['Water','Food','Torch','Medicine','Blanket','First Aid'].map(chip => (
                              <button key={chip} onClick={() => setItemName(chip)} className={`flex-shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition-all ${itemName === chip ? 'border-saffron-600 bg-saffron-600 text-white shadow-sm' : 'border-stone-200 bg-stone-50 text-stone-700 hover:bg-stone-100'}`}>{chip}</button>
                            ))}
                          </div>
                        ...
                        <GroupPanel session={session} profile={profile} groupAction={groupAction} setGroupAction={setGroupAction} registration={registration} setRegistration={setRegistration} registeredGroup={registeredGroup} familyProfiles={familyProfiles} liveLocations={liveLocations} locationError={locationError} onSubmit={() => void registerGroup()} />
                      ) : (
                        <>
                          <button onClick={() => setShowApplyModal(true)} className="w-full min-h-[48px] rounded-2xl bg-saffron-600 hover:bg-saffron-700 py-3 text-sm font-black text-white shadow-md transition">Apply for Seva ⚡</button>
                          {session ? <VolunteerDashboard session={session} profile={profile} role={role} approved={approved} loading={authLoading || profileLoading} nodes={nodes} sosAlerts={sosAlerts} sightings={sightings} setSosAlerts={setSosAlerts} setSightings={setSightings}/> : <p className="mt-3 text-xs font-semibold text-stone-500">Sign in to access volunteer features.</p>}
                        </>
                      )}
                    </div>
                  </div>
