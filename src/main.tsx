...
const COMMON_ITEM_CHIPS = ['Water', 'Food', 'Torch', 'Medicine', 'Blanket', 'First Aid', 'Phone charger', 'Raincoat'];
...
function isParticipant(item: ItemRequest, userId?: string) {
  return Boolean(userId && (item.requester_id === userId || item.accepted_by === userId));
}

function getPeerMeetingPoint(item: ItemRequest, userId?: string) {
  if (!isParticipant(item, userId) || item.status !== 'accepted') return undefined;
  if (item.requester_id === userId) {
    return item.accepter_lat !== undefined && item.accepter_lng !== undefined
      ? { lat: item.accepter_lat, lng: item.accepter_lng, label: 'Helper location' }
      : undefined;
  }
  return item.lat !== undefined && item.lng !== undefined
    ? { lat: item.lat, lng: item.lng, label: 'Requester location' }
    : undefined;
}
...
type AppView = 'pilgrim' | 'news' | 'sos_mesh' | 'helplines' | 'admin';
...
function App() {
  ...
  const [view, setView] = useState<AppView>(() => {
    ...
    if (location.hash === '#/helplines') return 'helplines';
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      else if (location.hash === '#/helplines') setView('helplines');
  ...
  const changeView = (nextView: AppView) => {
    ...
    else if (nextView === 'helplines') window.location.hash = '#/helplines';
  ...
  useEffect(() => {
    ...
    items.filter((item) => ['open', 'accepted'].includes(item.status ?? 'open') && !isExpiredOpenRequest(item)).forEach((item) => {
      const mine = item.requester_id === currentMemberId;
      const helping = item.accepted_by === currentMemberId;
      if ((item.status ?? 'open') === 'open' && item.lat !== undefined && item.lng !== undefined) {
        L.marker([item.lat, item.lng], {
          icon: L.divIcon({ className: '', html: `<div style="background:${mine ? '#2563eb' : '#16a34a'};color:#fff;border-radius:9999px;width:30px;height:30px;display:flex;align-items:center;justify-content:center;font-size:15px;box-shadow:0 1px 5px rgba(0,0,0,.35)">🤝</div>`, iconSize: [30, 30], iconAnchor: [15, 15] })
        })
          .bindPopup(`<strong>${mine ? 'Your request' : 'Pilgrim needs'}</strong><br/>${item.item_name}<br/>${mine ? 'Waiting for a helper' : 'Open request'}`)
          .addTo(markerLayerRef.current!);
      }
      const meetingPoint = getPeerMeetingPoint(item, currentMemberId);
      if (meetingPoint) {
        L.marker([meetingPoint.lat, meetingPoint.lng], {
          icon: L.divIcon({ className: '', html: '<div style="background:#ea580c;color:#fff;border-radius:9999px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 1px 5px rgba(0,0,0,.35)">📍</div>', iconSize: [32, 32], iconAnchor: [16, 16] })
        })
          .bindPopup(`<strong>${meetingPoint.label}</strong><br/>Peer lending: ${item.item_name}`)
          .addTo(markerLayerRef.current!);
        const myLat = position?.coords.latitude;
        const myLng = position?.coords.longitude;
        if (myLat !== undefined && myLng !== undefined) {
          L.polyline([[myLat, myLng], [meetingPoint.lat, meetingPoint.lng]], { color: '#ea580c', weight: 4, dashArray: '8 8' })
            .bindPopup(`Meet for ${item.item_name}`)
            .addTo(markerLayerRef.current!);
        }
      } else if ((mine || helping) && item.status === 'accepted' && item.lat !== undefined && item.lng !== undefined) {
        L.marker([item.lat, item.lng], {
          icon: L.divIcon({ className: '', html: '<div style="background:#ea580c;color:#fff;border-radius:9999px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 1px 5px rgba(0,0,0,.35)">📍</div>', iconSize: [32, 32], iconAnchor: [16, 16] })
        })
          .bindPopup(`<strong>Peer lending meetup</strong><br/>${item.item_name}`)
          .addTo(markerLayerRef.current!);
      }
    });
  ...
  }, [view, nodes, reports, trafficReports, groupNodes, liveLocations, items, session, currentMemberId, role, familyProfiles, position?.coords]);
  ...
  const acceptedPeerRequests = useMemo(() => items.filter((item) => item.status === 'accepted' && isParticipant(item, currentMemberId)), [items, currentMemberId]);
  ...
  async function requestItem() {
    ...
    if (!position) {
      setNotice({ type: 'error', text: 'Share your location before sending a lending request so helpers can find you.' });
      return;
    }
    if (!itemName.trim() || !currentMemberId || myActiveRequest) return;
    const result = await queueWrite<ItemRequest>('item_requests', { requester_id: currentMemberId, item_name: itemName.trim(), lat: position?.coords.latitude, lng: position?.coords.longitude, status: 'open' });
    setItemName(''); setItems((r) => [result.serverRecord ?? result.localRecord, ...r.filter((i) => i.id !== result.localRecord.id)]);
    setNotice({ type: 'success', text: 'Request sent. Other pilgrims will see it on their home screen.' });
  }
  ...
  async function acceptItem(item: ItemRequest) {
    ...
    if (!position) {
      setNotice({ type: 'error', text: 'Share your location before accepting so the requester can find you.' });
      return;
    }
    ...
    setNotice({ type: 'success', text: 'Offer accepted. Both pilgrims can now use the map to meet.' });
  }
...
function LendingPanel({ itemName, setItemName, myActiveRequest, sortedPrimaryItems, acceptedPeerRequests, currentMemberId, position, onRequestItem, onAcceptItem, onCompleteItem, onCancelItem, onUnacceptItem }: { itemName: string; setItemName: React.Dispatch<React.SetStateAction<string>>; myActiveRequest?: ItemRequest; sortedPrimaryItems: { item: ItemRequest; distance?: number }[]; acceptedPeerRequests: ItemRequest[]; currentMemberId?: string; position?: GeolocationPosition; onRequestItem: () => void; onAcceptItem: (item: ItemRequest) => void; onCompleteItem: (item: ItemRequest) => void; onCancelItem: (item: ItemRequest) => void; onUnacceptItem: (item: ItemRequest) => void }) {
  return <div className="space-y-4">
    <div className="rounded-2xl border border-saffron-200 bg-saffron-50 p-3">
      <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-saffron-600 text-lg text-white">🤝</span><div><h3 className="text-sm font-extrabold text-stone-900">Peer-to-peer lending</h3><p className="mt-1 text-xs text-stone-600">Send a request for an item you need. All signed-in pilgrims see open requests on the home screen and can offer help.</p></div></div>
      <div className="mt-3 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">{COMMON_ITEM_CHIPS.map((chip) => <button key={chip} type="button" onClick={() => setItemName(chip)} className={`flex-shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition-colors ${itemName === chip ? 'border-saffron-600 bg-saffron-600 text-white' : 'border-cream-200 bg-white text-stone-600'}`}>{chip}</button>)}</div>
      <label className="mb-1.5 block text-xs font-semibold text-stone-600">What do you need?</label>
      <input className="w-full min-h-[44px] rounded-xl border border-cream-200 bg-white px-3.5 py-3 text-sm text-stone-900 placeholder:text-stone-400 focus:border-saffron-600 focus:ring-2 focus:ring-saffron-600/20 focus:outline-none" value={itemName} onChange={(e) => setItemName(e.target.value)} placeholder="e.g. water bottle, torch..."/>
      {!position && <p className="mt-2 text-xs font-semibold text-amber-700">Share location before sending or accepting a request so both pilgrims can meet on the map.</p>}
      <button disabled={Boolean(myActiveRequest) || !itemName.trim()} onClick={onRequestItem} className="mt-3 w-full min-h-[48px] rounded-xl bg-saffron-600 py-3 text-sm font-bold text-white shadow-sm disabled:opacity-60">{myActiveRequest ? 'You already have an active request' : 'Send request to all pilgrims'}</button>
    </div>
    {acceptedPeerRequests.length > 0 && <div className="rounded-2xl border border-orange-200 bg-orange-50 p-3"><h4 className="text-xs font-extrabold uppercase tracking-wide text-orange-700">Accepted meetup</h4><div className="mt-2 space-y-2">{acceptedPeerRequests.map((item, idx) => { const peer = getPeerMeetingPoint(item, currentMemberId); const mapUrl = directionsUrl(peer?.lat, peer?.lng); return <div key={item.id ?? idx} className="rounded-xl bg-white p-3"><div className="flex items-center justify-between gap-2"><p className="text-sm font-extrabold text-stone-900">{item.item_name}</p><span className="rounded-full bg-orange-100 px-2 py-1 text-[11px] font-bold text-orange-700">{item.requester_id === currentMemberId ? 'Helper accepted' : 'You accepted'}</span></div><p className="mt-1 text-xs text-stone-600">Both pilgrims can now see the meetup location on the map.</p><div className="mt-2 flex flex-wrap gap-2">{mapUrl && <a href={mapUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-orange-600 px-3 py-1.5 text-xs font-bold text-white">Open directions</a>}{item.requester_id === currentMemberId ? <button onClick={() => onCompleteItem(item)} className="rounded-lg border border-green-600 px-3 py-1.5 text-xs font-bold text-green-700">Mark received</button> : <><button onClick={() => onCompleteItem(item)} className="rounded-lg border border-green-600 px-3 py-1.5 text-xs font-bold text-green-700">Mark delivered</button><button onClick={() => onUnacceptItem(item)} className="rounded-lg border border-stone-300 px-3 py-1.5 text-xs font-bold text-stone-600">Can’t help</button></>}</div></div>; })}</div></div>}
    <div><div className="mb-2 flex items-center justify-between"><h4 className="text-sm font-extrabold text-stone-900">Requests on home screen</h4><span className="rounded-full bg-cream-100 px-2 py-1 text-xs font-bold text-stone-600">{sortedPrimaryItems.length}</span></div><div className="space-y-2">{sortedPrimaryItems.slice(0, 6).map(({ item, distance }, idx) => <div key={item.id ?? idx} className="rounded-xl border border-cream-200 bg-white px-3 py-2"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-extrabold text-stone-900">{item.item_name}</p><p className="text-xs text-stone-500">{item.requester_id === currentMemberId ? 'Your request' : `Pilgrim needs this · ${formatDistance(distance)}`}</p></div><span className={`rounded-full px-2 py-1 text-[11px] font-bold ${item.status === 'accepted' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}>{item.status === 'accepted' ? 'Accepted' : 'Open'}</span></div><div className="mt-2 flex flex-wrap items-center gap-2">{item.status !== 'accepted' && item.requester_id !== currentMemberId && <button onClick={() => onAcceptItem(item)} className="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-bold text-white">I have this item</button>}{item.requester_id === currentMemberId && item.status === 'open' && <button onClick={() => onCancelItem(item)} className="rounded-lg border border-stone-300 px-3 py-1.5 text-xs font-bold text-stone-600">Cancel request</button>}{item.status === 'accepted' && !isParticipant(item, currentMemberId) && <span className="text-xs font-semibold text-stone-500">Already being helped</span>}</div></div>)}{sortedPrimaryItems.length === 0 && <p className="rounded-xl bg-saffron-50 p-3 text-xs text-stone-500">No open peer lending requests right now.</p>}</div></div>
  </div>;
}
