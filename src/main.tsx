...
function App() {
  ...
  useEffect(() => {
    ...
    void supabase.from('broadcast_messages').select('*').eq('active', true).order('created_at', { ascending: false }).limit(5).then(({ data }) => data && (void cacheRows('broadcast_messages', data), setBroadcastMessages(data as BroadcastMessage[]))); 
    ...
    const channel = supabase.channel(`vari-live-${Math.random().toString(36).slice(2)}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'broadcast_messages' }, () => void supabase.from('broadcast_messages').select('*').eq('active', true).order('created_at', { ascending: false }).limit(5).then(({ data }) => data && setBroadcastMessages(data as BroadcastMessage[])))
  ...
  const activeBroadcast = useMemo(() => broadcastMessages.find((message) => message.active !== false && (!message.expires_at || new Date(message.expires_at).getTime() > Date.now())), [broadcastMessages]);
  ...
  return (
    <main className="min-h-screen bg-saffron-50 text-stone-900">
      ...
      {activeBroadcast && <div className="sticky top-0 z-[60] overflow-hidden border-y border-amber-300 bg-amber-100 py-2 text-amber-950 shadow-sm" role="status" aria-live="polite"><div className="broadcast-marquee whitespace-nowrap text-sm font-extrabold"><span className="mr-10">📢 {activeBroadcast.message}</span><span>📢 {activeBroadcast.message}</span></div></div>}
